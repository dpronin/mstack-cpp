...
struct ipv4_header_t {
        ...
        static ipv4_header_t consume_from_net(std::byte* ptr) {
                ...
                ipv4_header.hlen                    = version_header_length & 0xF;
        ...
        void produce_to_net(std::byte* ptr) const {
                utils::produce_to_net<uint8_t>(ptr, version << 4 | hlen);
        ...
        friend std::ostream& operator<<(std::ostream& out, ipv4_header_t const& m) {
                fmt::format_to(std::ostream_iterator<char>(out),
                               ...
                               static_cast<uint8_t>(m.version), static_cast<uint8_t>(m.hlen), m.tos,
                               m.total_length, m.id, static_cast<uint8_t>(m.NOP),
                               static_cast<uint8_t>(m.DF), static_cast<uint8_t>(m.MF),
                               static_cast<uint16_t>(m.frag_offset), m.ttl, m.proto_type,
                               m.header_chsum, m.src_addr, m.dst_addr);
                return out;
        }
};
