...
void tcp_transmit::tcp_send_ack() {}
void tcp_transmit::tcp_send_syn_ack() {}
void tcp_transmit::tcp_send_rst() {}
void tcp_transmit::tcp_send_ctl() {}

int tcp_transmit::generate_iss() { return 0; }

bool tcp_transmit::tcp_handle_close_state(std::shared_ptr<tcb_t> in_tcb, tcp_packet_t& in_packet) {
        tcp_header_t in_tcp{
                tcp_header_t::consume(in_packet.buffer->get_pointer()),
        };
        /**
...
bool tcp_transmit::tcp_handle_listen_state(std::shared_ptr<tcb_t> in_tcb, tcp_packet_t& in_packet) {
        //  If the state is LISTEN then
        if (in_tcb->state_ != TCP_LISTEN) {
                return false;
        }

        tcp_header_t in_tcp{tcp_header_t::consume(in_packet.buffer->get_pointer())};

        spdlog::debug("[TCP LISTEN] {}", in_tcp);
        ...
        if (in_tcp.SYN == 1) {
                ...
                in_tcb->receive_.next        = in_tcp.seq_no + 1;
                in_tcb->receive_.window      = 0xFAF0;
                in_tcb->send_.next           = iss + 1;
                in_tcb->send_.unacknowledged = iss;
                in_tcb->next_state_          = TCP_SYN_RECEIVED;
                in_tcb->active_self();
...
bool tcp_transmit::tcp_handle_syn_sent(std::shared_ptr<tcb_t> in_tcb, tcp_packet_t& in_packet) {
        // If the state is SYN-SENT then
        if (in_tcb->state_ != TCP_SYN_SENT) {
                return false;
        }
...
bool tcp_transmit::tcp_check_segment(std::shared_ptr<tcb_t> in_tcb, tcp_packet_t& in_packet) {
        /**
         *  SYN-RECEIVED STATE
         *  ESTABLISHED STATE
         *  FIN-WAIT-1 STATE
        ...
        switch (in_tcb->state_) {
                ...
                case TCP_LAST_ACK:
                case TCP_TIME_WAIT:
                        if (segment_length == 0 && in_tcb->receive_.window == 0) {
                                if (in_tcp.seq_no == in_tcb->receive_.next) {
                                        return true;
                                } else {
                                        return false;
                                }
                        ...
                        if (segment_length == 0 && in_tcb->receive_.window > 0) {
                                ...
                                if (in_tcb->receive_.next <= in_tcp.seq_no &&
                                    in_tcp.seq_no <
                                            in_tcb->receive_.next + in_tcb->receive_.window) {
                                        return true;
                                } else {
                                        return false;
                                }
                        ...
                        if (segment_length > 0 && in_tcb->receive_.window > 0) {
                                ...
                                if ((in_tcb->receive_.next <= in_tcp.seq_no &&
                                     in_tcb->receive_.next + in_tcb->receive_.window) ||
                                    (in_tcb->receive_.next <= in_tcp.seq_no + segment_length - 1 &&
...
void tcp_transmit::tcp_in(std::shared_ptr<tcb_t> in_tcb, tcp_packet_t&& in_packet) {
        spdlog::debug("[TCP] [CHECK TCP_CLOSED] {}", *in_tcb);
        if (in_tcb->state_ == TCP_CLOSED && tcp_handle_close_state(in_tcb, in_packet)) {
                return;
        }

        spdlog::debug("[TCP] [CHECK TCP_LISTEN] {}", *in_tcb);
        if (in_tcb->state_ == TCP_LISTEN && tcp_handle_listen_state(in_tcb, in_packet)) {
                return;
        }

        spdlog::debug("[TCP] [CHECK TCP_SYN_SENY] {}", *in_tcb);
        if (in_tcb->state_ == TCP_SYN_SENT && tcp_handle_syn_sent(in_tcb, in_packet)) {
                return;
        }

        tcp_header_t in_tcp{tcp_header_t::consume(in_packet.buffer->get_pointer())};

        spdlog::debug("[TCP] [PROCESS 1] {}", *in_tcb);
        ...
        spdlog::debug("[TCP] [PROCESS 2] {}", *in_tcb);
        ...
        spdlog::debug("[TCP] [PROCESS 3] {}", *in_tcb);
         ...
         *  current connection.
         */
        spdlog::debug("[TCP] [PROCESS 4] {}", *in_tcb);
        ...
        spdlog::debug("[TCP] [PROCESS 5] {}", *in_tcb);
        if (in_tcp.ACK) {
                switch (in_tcb->state_) {
                        /**
                         *  SYN-RECEIVED STATE
                         ...
                         */
                        case TCP_SYN_RECEIVED:
                                if (in_tcb->send_.unacknowledged <= in_tcp.ack_no &&
                                    in_tcp.ack_no <= in_tcb->send_.next) {
                                        in_tcb->state_      = TCP_ESTABLISHED;
                                        in_tcb->next_state_ = TCP_ESTABLISHED;
                        ...
                        case TCP_CLOSING:
                                if (in_tcb->send_.unacknowledged < in_tcp.ack_no &&
                                    in_tcp.ack_no <= in_tcb->send_.next) {
                                        in_tcb->send_.unacknowledged = in_tcp.ack_no;
                                        // TODO: update windows
                                }

                                if (in_tcp.ack_no < in_tcb->send_.unacknowledged) { /* ignore */
                                }

                                if (in_tcp.ack_no > in_tcb->send_.next) {
                                        in_tcb->active_self();
                                        return;
                                }
                                 ...
                                 */
                                if (in_tcb->state_ == TCP_FIN_WAIT_1) {
                                        in_tcb->next_state_ = TCP_FIN_WAIT_2;
                                }
                                 ...
                                 * the TCB.
                                 */
                                if (in_tcb->state_ == TCP_FIN_WAIT_2) {
                                        // * CLOSE FINISH
                                }
                                /**
                                 *  CLOSE-WAIT STATE
                                ...
                                if (in_tcb->state_ == TCP_CLOSE_WAIT) {
                                }
                                /**
                                 *  CLOSING STATE
                                 *      In addition to the processing for the ESTABLISHED
                                 ...
                                 */
                                if (in_tcb->state_ == TCP_CLOSING) {
                                        in_tcb->next_state_ = TCP_TIME_WAIT;
                                }
                                break;
                        /**
                         *  LAST-ACK STATE
        ...
        spdlog::debug("[TCP] [PROCESS 6] {}", *in_tcb);
        ...
        spdlog::debug("[TCP] [PROCESS 7] {}", *in_tcb);
        ...
        if (segment_len > 0) {
                switch (in_tcb->state_) {
                        ...
                        case TCP_FIN_WAIT_1:
                        case TCP_FIN_WAIT_2: {
                                spdlog::debug("[RECEIVE DATA] {}", segment_len);

                                in_tcb->receive_.next += segment_len;
                                ...
                                if (auto pkt{raw_packet{.buffer = std::move(out_buffer)}};
                                    !in_tcb->on_data_receive_.empty()) {
                                        auto cb{std::move(in_tcb->on_data_receive_.front())};
                                        in_tcb->on_data_receive_.pop();
                                        in_tcb->io_ctx_.post(
                                                [cb          = std::move(cb),
                                                 pkt_wrapper = std::make_shared<raw_packet>(
                                                         std::move(pkt))] mutable {
                                                        cb(std::move(*pkt_wrapper));
                                                });
                                } else {
                                        in_tcb->receive_queue_.push(std::move(pkt));
                                }
        ...
        spdlog::debug("[TCP] [PROCESS 8] {}", *in_tcb);
        ...
        if (in_tcp.FIN == 1) {
                switch (in_tcb->state_) {
                        ...
                        case TCP_SYN_RECEIVED:
                        case TCP_ESTABLISHED:
                                in_tcb->receive_.next += 1;
                                in_tcb->next_state_ = TCP_CLOSE_WAIT;
                                in_tcb->active_self();
                                /**
                                 *  FIN-WAIT-1 STATE
                                 ...
                                 */
                        case TCP_FIN_WAIT_1:
                                if (in_tcb->next_state_ == TCP_FIN_WAIT_2) {
                                        in_tcb->next_state_ = TCP_TIME_WAIT;
                                } else {
                                        in_tcb->next_state_ = TCP_CLOSING;
                                }
                                /**
                                 *  FIN-WAIT-2 STATE
                ...
                spdlog::debug("[TCP] [PROCESS 9] {}", *in_tcb);
        }
}
